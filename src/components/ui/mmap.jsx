...
import { createContext, useContext, useEffect, useRef, useState, lazy } from "react";
import { renderToString } from "react-dom/server";
import { useMap, useMapEvents } from "react-leaflet";

const LeafletMapContainer = lazy(async () => (await import("react-leaflet")).MapContainer, { ssr: false });

const LeafletTileLayer = lazy(async () => (await import("react-leaflet")).TileLayer, { ssr: false });
const LeafletMarker = lazy(async () => (await import("react-leaflet")).Marker, { ssr: false });
const LeafletPopup = lazy(async () => (await import("react-leaflet")).Popup, { ssr: false });
const LeafletTooltip = lazy(async () => (await import("react-leaflet")).Tooltip, { ssr: false });
const LeafletCircle = lazy(async () => (await import("react-leaflet")).Circle, { ssr: false });
const LeafletCircleMarker = lazy(async () => (await import("react-leaflet")).CircleMarker, { ssr: false });
const LeafletPolyline = lazy(async () => (await import("react-leaflet")).Polyline, { ssr: false });
const LeafletPolygon = lazy(async () => (await import("react-leaflet")).Polygon, { ssr: false });
const LeafletRectangle = lazy(async () => (await import("react-leaflet")).Rectangle, { ssr: false });
const LeafletLayerGroup = lazy(async () => (await import("react-leaflet")).LayerGroup, { ssr: false });
const LeafletFeatureGroup = lazy(async () => (await import("react-leaflet")).FeatureGroup, { ssr: false });

function Map({ zoom = 15, className, ...props }) {
  return (
    <LeafletMapContainer
      zoom={zoom}
  ...
  useEffect(() => {
    ...
    if (tileLayers.length > 0 && !selectedTileLayer) {
      const validDefaultValue =
        defaultTileLayer && tileLayers.some(layer => layer.name === defaultTileLayer) ? defaultTileLayer : tileLayers[0].name;
...
function MapLayersControl({ tileLayersLabel = "Map Type", layerGroupsLabel = "Layers", className, ...props }) {
...
function MapZoomControl({ className, ...props }) {
  const map = useMap();
...
function MapLocateControl({ className, watch = false, onLocationFound, onLocationError, ...props }) {
  const map = useMap();
...
function MapDrawControl({ className, onLayersChange, ...props }) {
